...
export function getGlobalExcalidrawAPI(): any {
  return globalExcalidrawAPI;
}

export function useMenuHandler() {
  const { loadDirectory, createNewFile, saveCurrentFile, activeFile } =
    useFileStore();
  const { toggleSidebar, preferences } = useUIStore();
  ...
  const handleClearRecent = async () => {
    ...
    useUIStore.getState().setPreferences(newPrefs);
  };
